...
from health import HealthPellet
...
class Game:
    def __init__(self, screen):
        ...
        self.health_pellet_spawn_chance = 0.1  # 10% chance to spawn a health pellet
    ...
    def update(self):
        ...
        for pellet in self.health_pellets[:]:
            pellet.update()
            if pellet.rect.top > self.screen.get_rect().bottom:
                self.health_pellets.remove(pellet)
            elif pellet.rect.colliderect(self.player.rect):
                self.health_pellets.remove(pellet)
                self.gain_life()
        ...
        if current_time - self.last_spawn_time >= self.spawn_delay:
            self.spawn_enemy()
            if random.random() < self.health_pellet_spawn_chance:  # Add this line
                self.spawn_health_pellet()
    ...
    def spawn_health_pellet(self):
        pellet = HealthPellet(self.screen, self.enemy_speed)
        self.health_pellets.append(pellet)
